...
def main():
    ...
    plot_path = run_dir / "simulation_delay_linear.pdf"
    ...
    # -----------------------------
    # Plotting: analytical_total_delay vs sim_total_delay (linear y-axis)
    # -----------------------------
    ...
    ax.legend(handles=[sim_handle, ana_handle], loc="lower right", fontsize=10)

    ax.set_xlabel(r"$\lambda$, Arrival Rate (packets/s)", fontsize=14)
    ax.set_ylabel(r"$E[D]$, Average Delay (s)", fontsize=14)
